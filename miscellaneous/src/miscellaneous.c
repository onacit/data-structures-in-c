...
const char *ordinal_suffix(unsigned char const c) {
    switch (c % 10) {
        case 1:
...
int sprintf_ordinal_u(char *const s, unsigned int const c) {
    assert(s != NULL);
    return sprintf(s, "%u%s", c, ordinal_suffix(c));
}

int snprintf_ordinal_u(char *s, size_t n, unsigned int c) {
    assert(s != NULL);
    return snprintf(s, n, "%u%s", c, ordinal_suffix(c));
}

int sprintf_ordinal_lu(char *const s, unsigned long int const c) {
    assert(s != NULL);
    return sprintf(s, "%lu%s", c, ordinal_suffix(c));
}

int snprintf_ordinal_lu(char *s, size_t n, unsigned long int c) {
    assert(s != NULL);
    return snprintf(s, n, "%lu%s", c, ordinal_suffix(c));
}

int sprintf_ordinal_llu(char *const s, unsigned long long int const c) {
    assert(s != NULL);
    return sprintf(s, "%llu%s", c, ordinal_suffix(c));
}

int snprintf_ordinal_llu(char *s, size_t n, unsigned long long int c) {
    assert(s != NULL);
    return snprintf(s, n, "%llu%s", c, ordinal_suffix(c));
}
